...
async function main() {
  ...
  const Escrow = await hre.ethers.getContractFactory("ByzantineMilestoneEscrow");
  ...
  const artifact = await hre.artifacts.readArtifact("ByzantineMilestoneEscrow");
  const contractInfo = {
    address,
    abi: artifact.abi,
  ...
  console.log("\nByzantineMilestoneEscrow deployed to:", address);
  console.log("\nRoles");
