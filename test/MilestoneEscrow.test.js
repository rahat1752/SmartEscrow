...
describe("MilestoneEscrow", function () {
  ...
  it("prevents duplicate approval by same approver", async function () {
    ...
    ).to.be.revertedWith("Approver already approved this submission");
  });
  ...
  it("allows approver dispute, freelancer response, and resubmission with reset approvals", async function () {
    const { escrow, freelancer, approver1 } = await deployFixture();

    await escrow.connect(freelancer).submitMilestone(2, "ipfs://old-proof");
    await escrow.connect(approver1).approveMilestone(2);
    expect(await escrow.hasApprovedCurrentSubmission(2, approver1.address)).to.equal(true);

    await escrow.connect(approver1).raiseDispute(2, "Backend tests are missing");
    const dispute = await escrow.getLatestDispute(2);
    expect(dispute.exists).to.equal(true);
    expect(dispute.raisedBy).to.equal(approver1.address);
    expect(dispute.reason).to.equal("Backend tests are missing");

    await escrow.connect(freelancer).respondToDispute(2, "I will add unit tests and resubmit.");
    const responded = await escrow.getLatestDispute(2);
    expect(responded.freelancerResponse).to.equal("I will add unit tests and resubmit.");

    await escrow.connect(freelancer).resubmitMilestone(2, "ipfs://new-proof", "Added missing tests.");
    const milestone = await escrow.getMilestone(2);
    expect(milestone.status).to.equal(1); // Submitted again
    expect(milestone.approvalCount).to.equal(0);
    expect(milestone.proofURI).to.equal("ipfs://new-proof");
    expect(await escrow.hasApprovedCurrentSubmission(2, approver1.address)).to.equal(false);
  });
